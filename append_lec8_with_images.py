...
WITH INLINE IMAGES from extracted slide pages.

This script rebuilds Lecture 8 from scratch using the Lec1-7 doc as base,
inserting relevant slide images at strategic points within the summary sections.

Image path format: lec8_p{N}_0.jpeg (N=1 to 43)
"""
...
def img_path(page_num):
    """Return the image path for a given page number."""
    return os.path.join(IMG_DIR, f"lec8_p{page_num}_0.jpeg")

def get_image_stream(page_num):
    """Open image with Pillow, re-save to BytesIO as proper JPEG for python-docx."""
    path = img_path(page_num)
...
def add_image(page_num, width_inches=5.0):
    """Add an image from the extracted slides, re-saving via Pillow to fix headers."""
    stream = get_image_stream(page_num)
    if stream:
        try:
            doc.add_picture(stream, width=Inches(width_inches))
            print(f"  ✓ Inserted image: lec8_p{page_num}_0.jpeg")
        except Exception as e:
            print(f"  ✗ Failed to insert image p{page_num}: {e}")
    else:
        print(f"  ✗ Image not found: {img_path(page_num)}")
...
doc.save(OUT_PATH)
print(f"\n✓ Lecture 8 (Other Lichenoid Dermatoses) appended WITH IMAGES successfully!")
print(f"  Saved to: {OUT_PATH}")
print(f"  Total images inserted: 11 locations attempted")
